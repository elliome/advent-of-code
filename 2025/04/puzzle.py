    ...
    for x, char in enumerate(line):
        ...
        for relative_x in range(x + -1,  x + 2):
            for relative_y in range(y + -1,  y + 2):
                if f'{relative_x},{relative_y}' in grid:
                    neighbours.append(grid[f'{relative_x},{relative_y}'])
    ...
    for y, line in enumerate(lines):
        for x, char in enumerate(line):
            ...
            for relative_x in range(x + -1,  x + 2):
                for relative_y in range(y + -1,  y + 2):
                    if f'{relative_x},{relative_y}' in grid:
                        neighbour_count += grid[f'{relative_x},{relative_y}']
